...
import {
  ...
  Code2, Globe, FileText, CheckCircle2,
  FolderKanban, Building2, Award,
} from 'lucide-react';
...
export function Sidebar() {
  ...
  const section = (label: string, count: number, icon: React.ReactNode) => (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2 text-xs text-[#6E6E73]">
        {icon}
        {label}
      </div>
      <span className="text-xs font-semibold text-[#1D1D1F] bg-[#F5F5F7] px-2 py-0.5 rounded-md">
        {count}
      </span>
    </div>
  );
      ...
      <div className="w-[280px] flex-shrink-0 border-r border-[#E5E5EA] bg-white flex flex-col animate-in slide-in-from-left duration-200">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E5E5EA]">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-[#1D1D1F]">Profile Summary</span>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="space-y-2">
            {section('Master Resume', 1, <FileText className="w-3 h-3" />)}
            {section('Projects', projects.length, <FolderKanban className="w-3 h-3" />)}
            {section('Experience', experiences.length, <Building2 className="w-3 h-3" />)}
            {section('Achievements', achievements.length, <Award className="w-3 h-3" />)}
          </div>
